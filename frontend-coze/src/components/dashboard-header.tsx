    ...
    >
      ...
      <div className="relative z-10 mt-2 flex min-w-0 items-center gap-1.5 text-[11px] text-slate-300 sm:mt-0 sm:gap-6 sm:text-[12px]">
        ...
        <div className="flex shrink-0 items-center gap-1.5 border-l border-white/10 pl-1.5 sm:gap-3 sm:pl-3.5">
          ...
          {onShowDataConsole && (
            <button
              onClick={onShowDataConsole}
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-white/15 text-slate-200 hover:bg-white/10 hover:text-white active:scale-[0.98] transition-all"
              title="查看静态数据包状态与导入说明"
            >
              <Settings className="w-3.5 h-3.5" />
              <span className="hidden sm:inline">数据包</span><span className="sm:hidden">数据</span>
            </button>
          )}
          <span className="text-slate-300 max-w-[100px] truncate hidden sm:inline">公开访问</span>
        </div>
      </div>
    </header>
  );
