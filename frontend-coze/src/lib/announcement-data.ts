...
async function fetchStaticAnnouncements(): Promise<StaticAnnouncementsResponse> {
  const response = await fetch("/data/announcement_table.csv", { cache: "no-store" });
  if (!response.ok) {
    if (response.status === 404) throw new DataNotGeneratedError();
...
export class DataNotGeneratedError extends Error {
  constructor() {
    super("尚未生成看板数据，请替换 public/data 文件。");
