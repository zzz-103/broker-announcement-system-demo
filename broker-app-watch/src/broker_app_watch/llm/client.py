...
try:
    ...
    # Keep direct ``python -m broker_app_watch.cli`` execution working from
    # the App Watch directory while reusing the shared implementation.
    repository_root = Path(__file__).resolve().parents[4]
    if str(repository_root) not in sys.path:
        sys.path.insert(0, str(repository_root))
    from backend.llm_table.llm_client import (  # type: ignore[no-redef]
        LLMApiConfig,
        OpenAICompatibleClient,
        parse_json_text,
    )
