...
export function SavedConfigList({
  ...
  if (topics.length === 0) return (
    <div className="rounded-lg border border-dashed border-[#C8D7F0] bg-[#F8FAFD] py-10 text-center">
      ...
      <button type="button" onClick={onCreate} className="mt-4 inline-flex min-h-11 items-center gap-1.5 rounded-md bg-[#2563EB] px-3 py-2 text-xs font-semibold text-white hover:bg-[#1D4ED8] sm:min-h-0 sm:py-1.5"><Plus className="size-3.5" aria-hidden="true" />新建助手</button>
    </div>
  );
  return (
          ...
          <article key={topic.id} className="rounded-lg border border-[#E4EAF2] bg-white p-4 sm:p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="flex shrink-0 flex-wrap items-center gap-2">
                <button type="button" onClick={() => onRun(topic)} disabled={busy} className="inline-flex min-h-11 items-center gap-1 rounded-md bg-[#2563EB] px-3 py-2 text-[11px] font-semibold text-white hover:bg-[#1D4ED8] disabled:cursor-not-allowed disabled:opacity-45 sm:min-h-0 sm:py-1.5"><Play className="size-3" aria-hidden="true" />生成报告</button>
                <button type="button" onClick={() => onEdit(topic)} className="inline-flex min-h-11 items-center gap-1 rounded-md border border-[#D0D5DD] px-3 py-2 text-[11px] font-semibold text-[#475467] hover:bg-[#F8FAFC] sm:min-h-0 sm:px-2.5 sm:py-1.5"><Edit3 className="size-3" aria-hidden="true" />编辑</button>
                <button type="button" onClick={() => onDelete(topic)} disabled={busy} className="inline-flex min-h-11 items-center gap-1 rounded-md border border-transparent px-3 py-2 text-[11px] font-semibold text-[#B42318] hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-40 sm:min-h-0 sm:px-2.5 sm:py-1.5"><Trash2 className="size-3" aria-hidden="true" />删除</button>
              </div>
            </div>
          </article>
        );
