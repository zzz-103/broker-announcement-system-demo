...
import { AUDIENCE_OPTIONS, AUDIENCE_RESEARCH_SUGGESTIONS, FIELD_INPUT_CLASS, FOCUS_TAG_LIMIT, REPORT_LENGTH_OPTIONS, TIME_RANGE_OPTIONS } from "./custom-intelligence-constants";
...
export function SavedConfigDialog({
  ...
  const focusSuggestions = AUDIENCE_RESEARCH_SUGGESTIONS[draft.audience] ?? [];
  return (
      ...
      <DialogContent className="flex max-h-[calc(100dvh-1rem)] w-[calc(100%-1rem)] max-w-[720px] flex-col gap-0 overflow-hidden border-[#D9E2EC] bg-white p-0 sm:max-h-[calc(100dvh-2rem)] sm:w-[calc(100%-2rem)]">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-4 py-4 sm:px-6">
          <div className="space-y-4">
            ...
            <fieldset><legend className="mb-1.5 flex w-full justify-between text-xs font-semibold text-[#344054]"><span>关注方向</span><span className="text-[10px] font-normal text-[#98A2B3]">多选 · {draft.focus_tags.length}/{FOCUS_TAG_LIMIT}</span></legend><div className="flex flex-wrap gap-1.5">{focusSuggestions.map((tag) => { const selected = draft.focus_tags.includes(tag); return <button key={tag} type="button" aria-pressed={selected} disabled={!selected && draft.focus_tags.length >= FOCUS_TAG_LIMIT} onClick={() => update("focus_tags", selected ? draft.focus_tags.filter((item) => item !== tag) : [...draft.focus_tags, tag])} className={`rounded-full border px-2.5 py-1 text-[11px] transition-colors ${selected ? "border-[#4F7CFF] bg-[#EEF4FF] text-[#2455AC]" : "border-[#E4EAF2] text-[#667085] hover:border-[#9FB9E8]"} disabled:opacity-40`}>{tag}</button>; })}</div><p className="mt-1.5 text-[10px] text-[#98A2B3]">可直接选择关注方向，也可自行输入，系统会自动识别并梳理需求。</p></fieldset>
            <div><label htmlFor="assistant-config-focus" className="mb-1.5 flex items-center justify-between text-xs font-semibold text-[#344054]"><span>关注内容</span><span className="text-[10px] font-normal text-[#98A2B3]">可选补充</span></label><textarea id="assistant-config-focus" value={draft.focus} onChange={(event) => update("focus", event.target.value)} rows={3} maxLength={1000} placeholder="补充需要重点比较的机构、场景或具体问题" className="w-full resize-y rounded-md border border-[#D0D5DD] px-3 py-2.5 text-sm leading-6 outline-none focus:border-[#4F7CFF] focus:ring-2 focus:ring-[#4F7CFF]/15" /></div>
