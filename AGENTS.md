...
- 已建立统一 `dashboard-data` 标准数据包（Manifest、版本、统计、SHA-256），完整后端提供受保护读取接口和管理员 ZIP 导出入口
- 正式前端与 `frontend-coze` 共用标准化数据类型；完整前端读取标准数据包，纯前端已改为无登录、无数据库、无后端依赖的 Hash 静态看板
- `frontend-coze/public/dashboard-data/` 已提供可直接复制的样例包，支持当前会话目录导入、数据状态查看、缺失/损坏/版本错误中文提示和 ETag 缓存
- 数据包导出层已预计算基础统计与筛选元数据；纯前端启动时校验安全文件名、结构、记录数和 SHA-256，并支持子路径静态部署
- 已增加 `scripts/export_dashboard_data.py`、纯前端数据包复制/校验脚本及架构说明；复制命令共用校验器并采用临时目录切换，未导出用户数据库、密码、Token、服务器路径或 LLM 配置
- 纯前端按招采、App 更新和数据控制台按需加载数据，手动刷新会清理静态包缓存；项目明细和 App 明细保留分页/分段展示
