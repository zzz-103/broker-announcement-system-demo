...
管理员可在管理控制台“前端数据包”导出 ZIP，或在仓库根目录运行
`python scripts/export_dashboard_data.py --zip`。数据包包含
`manifest.json`、`overview.json`、`filters.json`、`tender_projects.json`、
`app_updates.json` 和 `ai_analysis.json`，由 FastAPI 的 `GET /api/dashboard-data/*`
接口提供给正式前端；数据包不包含用户表、密码、Token、原始路径或 LLM 配置。
导出层统一校验版本、文件安全性、字节数、SHA-256、记录数和关键结构。
