...
from backend.broker_sources.selector import prepare_selected_sources, select_documents
...
class SourceSelectionTests(unittest.TestCase):
    ...
    def test_empty_generation_preserves_last_known_good_output(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            output = root / "selected"
            sentinel = output / "procurement" / "notices" / "old.md"
            sentinel.parent.mkdir(parents=True)
            sentinel.write_text("last known good", encoding="utf-8")

            with self.assertRaisesRegex(ValueError, "previous selected generation was preserved"):
                prepare_selected_sources(
                    project_root=root,
                    configs={},
                    official_root=root / "official",
                    cfcpn_procurement_dir=root / "missing-procurement",
                    cfcpn_result_dir=root / "missing-result",
                    external_dir=root / "missing-external",
                    output_root=output,
                )
